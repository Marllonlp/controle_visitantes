...
class Visitante(models.Model):
    ...
    STATUS_VISITANTE = [
        ("AGUARDANDO", "Aguardando autorizão"),
        ("EM_VISITA", "Em visita"),
        ("FINALIZADO", "Visita finalzada")
    ]
    ...
    morador_responsavel = models.CharField(
        verbose_name="Nome do morador responsavel pela entrada do visitante",
        max_length=194,
        blank=True
    )
    ...
    registrado_por = models.ForeignKey(
        "porteiros.Porteiro",
        verbose_name="Porteiro responsavel pelo regitro",
        on_delete=models.PROTECT
    )

    def get_horario_saido(self):
        if self.horario_saido:
            return self.horario_saido
        return "Horário de não registrado"
